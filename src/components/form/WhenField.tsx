...
import { t } from '@/i18n';
import { calendarNames, fmtDate, isoDaysFromToday, todayIso as localTodayIso } from '@/utils/datetime';
import { TIME_PRESETS } from '@/constants/app';
...
interface WhenFieldProps {
  ...
  /** Earliest hour still ahead today: time presets starting before it are
      disabled while "today" is selected (a past departure is rejected). */
  minHourToday?: number;
}

/** Date chooser (today/tomorrow/+2/custom calendar) + time presets + range. */
export function WhenField({ date, onDate, tStart, tEnd, onTime, minHourToday }: WhenFieldProps) {
  const dateOpts = [0, 1, 2].map((i) => ({
    iso: isoDaysFromToday(i),
    label: i === 0 ? t('form.today') : i === 1 ? t('form.tomorrow') : t('form.dayAfter'),
  }));
  const { months, weekdays } = calendarNames();
  ...
  const initRef = customActive ? new Date(date + 'T00:00') : new Date();
    ...
    <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          <button onClick={() => {
            ...
            <span style={{ fontSize: 12, fontWeight: customActive ? 600 : 400, color: customActive || calOpen ? T.amber : T.muted,
              whiteSpace: 'nowrap' }}>
              {customActive ? fmtDate(new Date(date + 'T00:00')) : t('form.other')}
            </span>
          </button>
        </div>
      </div>
      ...
      {calOpen && (
        <div style={{ marginBottom: 12,
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{months[calYM.m]} {calYM.y}</span>
            <button onClick={() => shiftMonth(1)} style={{ width: 30, height: 30, borderRadius: 9,
              border: `1px solid ${T.border}`, background: 'transparent', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3 }}>
            {buildCells(calYM.y, calYM.m).map((d, i) => {
              ...
              const sel = date === iso; const today = iso === todayIso; const past = iso < todayIso;
              return (
                <button key={iso} disabled={past} onClick={() => { onDate(iso); setCalOpen(false); }} style={{
                  ...
                  border: today && !sel ? `1px solid ${T.amber}66` : '1px solid transparent',
                  background: sel ? T.amber : 'transparent',
                  color: sel ? '#1a1205' : past ? T.muted + '44' : T.text,
                  fontSize: 13, fontWeight: sel ? 700 : today ? 600 : 400,
      ...
      <div style={{ display: 'flex', gap: 6, margin: '2px 0 12px' }}>
        {TIME_PRESETS.map((p) => {
          ...
          const passed = isToday && minHourToday != null && p.s < minHourToday;
          return (
            <button key={p.id} onClick={() => onTime(p.s, p.e)} disabled={passed} aria-pressed={!!on} style={{
              flex: 1, padding: '7px 0', borderRadius: 9, cursor: passed ? 'not-allowed' : 'pointer',
